...
#define bufffree(B)	((uint32_t)(LUAL_BUFFERSIZE - bufflen(B)))

#define LIMIT	(LUA_MINSTACK/2)


static int emptybuffer(luaL_Buffer* B) {
	uint32_t l = bufflen(B);
...
static void adjuststack(luaL_Buffer* B) {
	if (B->lvl > 1) {
		...
		uint32_t toplen = lua_strlen(L, -1);
		do {
			uint32_t l = lua_strlen(L, -(toget + 1));
...
LUALIB_API int luaL_loadbuffer(lua_State* L, const char* buff, uint32_t size,
	const char* name) {
	LoadS ls;
	ls.s = buff;
	ls.size = size;
